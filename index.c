...
int index_save(const Index *index) {
    ...
    if (!tmp) return -1;
    *tmp = *index;
    qsort(tmp->entries, tmp->count, sizeof(IndexEntry), cmp_entry);
...
//
// Returns 0 on success, -1 on error.int index_add(Index *index, const char *path) {
int index_add(Index *index, const char *path) {
    ...
    size_t read_sz = (sz > 0) ? fread(buf, 1, sz, f) : 0;
    fclose(f);

    ObjectID oid;
    if (object_write(OBJ_BLOB, buf, read_sz, &oid) != 0) { free(buf); return -1; }
    free(buf);
    ...
    e->mode = ((st.st_mode & S_IXUSR) || (st.st_mode & S_IXGRP) || (st.st_mode & S_IXOTH)) ? 100755 : 100644;
